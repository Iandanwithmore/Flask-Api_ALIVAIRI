...
class CActividad(FnBase):
    ...
    def print_psicologica(self, loPdf) -> bool:
        ...
        loPdf.set_xy(x + self.l_w * 0.18, y)
        tmp_arr1 = LADATOS[21].copy()
        tmp_arr2 = tmp_arr1.copy()
        tmp_arr3 = tmp_arr1.copy()
        tmp_arr4 = tmp_arr1.copy()
        tmp_arr1["MTABLA"] = LADATOS[21]["MTABLA"][0:3]
        tmp_arr2["MTABLA"] = LADATOS[21]["MTABLA"][3:6]
        tmp_arr3["MTABLA"] = LADATOS[21]["MTABLA"][6:8]
        tmp_arr4["MTABLA"] = LADATOS[21]["MTABLA"][8:10]
    ...
    def print_jsonactividad(self, loPdf) -> bool:
        ...
        loPdf.row(["PRESION ARTERIAL: ", self.paDatos[3]["CRESULT"]], [w3, w6])
        loPdf.row(
        ...
        loPdf.ln(self.l_h)
        loPdf.cell(w2, self.l_h, "HEMOGRAMA", 0, 0, "L")
        self.opt_v_coment(loPdf, w8, self.l_h, self.paDatos[5], self.paDatos[6])
        loPdf.ln(self.l_h)
        loPdf.cell(w2, self.l_h, "EXAMEN DE ORINA", 0, 0, "L")
        self.opt_v_coment(loPdf, w8, self.l_h, self.paDatos[7], self.paDatos[8])
        loPdf.ln(self.l_h)
        loPdf.row(
