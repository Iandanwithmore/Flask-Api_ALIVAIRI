...
def PDF_JsonActividad():
    ...
    for laFila in data:
        ...
        lmCie10 = dict()
        lmCie10["OCIE10"] = (
            " " if laFila["OCIE10"] == 0 else laFila["OCIE10"].replace(",", "\n")
        )

        lmExtra = dict()
        lmExtra["MCONCLU"] = (
            " " if laFila["MCONCLU"] == 0 else laFila["MCONCLU"].replace(",", "\n")
        )
        lmExtra["MRECOME"] = (
            " " if laFila["MRECOME"] == 0 else laFila["MRECOME"].replace(",", "\n")
        )
        lmExtra["MOBSERV"] = (
            " " if laFila["MOBSERV"] == 0 else laFila["MOBSERV"].replace(",", "\n")
        )
